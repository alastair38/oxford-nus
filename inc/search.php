...
function blockhaus_custom_form($placeholder = null, $post_type = null) {
	$form = '<form role="search" method="get" class="search-form xl:max-w-prose mx-auto text-sm leading-6 flex py-1 px-2 bg-base rounded-full ring-1 ring-offset-2 group ring-neutral-light-500 duration-200 focus-within:ring-contrast gap-2" action="' . home_url( '/' ) . '">
	<label class="w-full">
	...
	<button type="submit" class="search-submit px-2 bg-primary text-contrast rounded-full hover:text-base focus-visible:text-base hover:bg-contrast outline-none ring-1 ring-offset-2 ring-transparent focus-visible:ring-contrast focus-visible:bg-contrast transition-colors duration-200 cursor-pointer" aria-label="Submit search" value="Search">
